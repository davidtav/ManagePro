...
$id = $_GET['id'];
$query = "SELECT * FROM `colaborador` WHERE `id_colaborador`=$id";
...
<nav>
    ...
    <section class="colaborador">
        ...
        <h1 >Alteração de Dados do Colaborador</h1>
      
            <label>
                Nome:
                <input type="text" name="nome_colab" value="<?= $item['nome_colaborador'] ?>">
            </label>
            <label>
                Telefone:
                <input type="text" name="fone_colab" value="<?= $item['telefone'] ?>">
            </label>

            <label>
                e-mail:
                <input type="email" name="email_colab" value="<?= $item['email'] ?>">          
            </label>

            <label>
                Senha:
                <input type="text" name="senha_colab" value="<?= $item['senha'] ?>">
            </label>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query_update = "UPDATE `colaborador` SET `nome_colaborador`='$nome', 
`telefone`='$telefone', 
`email`='$email',
`senha`='$hash'
WHERE `id_colaborador`= $id";
    ...
    if ($resultado_update) {
        header("Location:sucesso_colab.php?mensagem=ok");
        exit();
    } else {
