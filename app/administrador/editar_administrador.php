...
<nav>
        ...
        <div class="menu_admin">
            <a href="lista_administrador.php">Lista de administradores</a>
    ...
    <section class="colaborador">
        ...
        <h1 >Alteração de Dados de Administrador</h1>
      
            <label>
                Nome do Administrador:
                <input type="text" name="nome_admin" value="<?= $item['nome_administrador'] ?>">
            </label>
            <label>
                Telefone:
                <input type="text" name="fone_admin" value="<?= $item['telefone'] ?>">
            </label>

            <label>
                e-mail:
                <input type="email" name="email_admin" value="<?= $item['email'] ?>">          
            </label>

            <label>
                Senha:
                <input type="text" name="senha_admin" value="<?= $item['senha'] ?>">
            </label>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query_update = "UPDATE `administrador` SET `nome_administrador`='$nome', 
`telefone`='$telefone', 
`email`='$email',
`senha`='$hash'
WHERE `id_administrador`= $id";
    ...
    if ($resultado_update) {
        header("Location:sucesso_admin.php?mensagem=ok");
        exit();
    } else {
